...
from configs._base_.datasets.dotav15_val_800 import img_norm_cfg, dataset_type
from configs._base_.datasets.dotav15_val_800 import data as src_data
...
common_pipeline = [
    ...
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels'],
         meta_keys=('filename', 'ori_filename', 'ori_shape',
                    'img_shape', 'pad_shape', 'scale_factor', 'flip',
                    'flip_direction', 'img_norm_cfg', 'tag', 'raw_img')
         )
]
strong_pipeline = [
    ...
    dict(type='DTToNumpy'),
    dict(type="ExtraAttrs", tag="unsup_strong", keep_raw=True),
]
weak_pipeline = [
    # dict(type='LoadImageFromFile'),
    # dict(type='LoadAnnotations', with_bbox=True),
    ...
    dict(
        ...
        direction=['horizontal', 'vertical', 'diagonal'],
        version=angle_version),
    dict(type="ExtraAttrs", tag="unsup_weak", keep_raw=True),
]
unsup_pipeline = [
    dict(type="LoadImageFromFile"),
    # dict(type="LoadAnnotations", with_bbox=True),
...
model = dict(
    type="RotatedFocalTeacher",
    model=detector,
    semi_loss=dict(type='RotatedFTLoss'),
    train_cfg=dict(
        iter_count=0,
        burn_in_steps=6400,
        sup_weight=1.0,
        ...
        logit_specific_weights=dict(
            loss_cls=1.0, loss_bbox=1.0, loss_centerness=1.0),
        region_ratio=0.01
    ),
